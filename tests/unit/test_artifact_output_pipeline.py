...
def resolved_config(workspace: Path) -> dict:
    return {
        ...
        "checkpointing": {"output_dir": "artifacts/checkpoints/EXP-0001"},
        "export": {"output_dir": "artifacts/export/EXP-0001"},
        "metrics": {"history_path": "artifacts/checkpoints/EXP-0001/metrics/history.jsonl"},
        "tracking": {"root_dir": "artifacts/tracking/EXP-0001"},
...
def test_layout_paths(workspace: Path) -> None:
    layout = ArtifactOutputLayout(workspace_root=workspace, experiment_id="EXP-0001")
    assert layout.adapter_dir == workspace / "models" / "adapters" / "EXP-0001"
    assert layout.merged_dir == workspace / "models" / "merged" / "EXP-0001"
    assert layout.export_dir == workspace / "models" / "exports" / "EXP-0001"
    assert layout.adapter_checkpoints_dir == (
        workspace / "training" / "cache" / "EXP-0001" / "checkpoints"
    )
    assert layout.experiment_dir == workspace / "experiments" / "EXP-0001"
    assert layout.metrics_history_path == (
        workspace / "experiments" / "EXP-0001" / "metrics" / "history.jsonl"
    )
...
def test_manager_rewrites_config_paths(resolved_config: dict, workspace: Path) -> None:
    ...
    assert updated["export"]["output_dir"] == str(workspace / "models" / "exports" / "EXP-0001")
    assert updated["metrics"]["history_path"] == str(
        workspace / "experiments" / "EXP-0001" / "metrics" / "history.jsonl"
    )
    assert updated["tracking"]["root_dir"] == str(
        workspace / "experiments" / "EXP-0001" / "logs" / "tracking"
    )
...
def test_no_canonical_layout_without_workspace() -> None:
    config = {"name": "EXP-0001", "checkpointing": {"output_dir": "artifacts/checkpoints"}}
...
def test_publish_adapter_from_checkpoint(workspace: Path, resolved_config: dict) -> None:
    ...
    ckpt = workspace / "training" / "cache" / "EXP-0001" / "checkpoints" / "checkpoint-100"
    ...
    assert artifact["identifier"] == "EXP-0001"
...
def test_atomic_publish_replaces_existing(workspace: Path, resolved_config: dict) -> None:
    ...
    ckpt1 = workspace / "training" / "cache" / "EXP-0001" / "checkpoints" / "checkpoint-50"
    _write_valid_checkpoint(ckpt1)
    manager.publish_adapter_from_checkpoint(ckpt1)

    ckpt2 = workspace / "training" / "cache" / "EXP-0001" / "checkpoints" / "checkpoint-100"
...
def test_cleanup_protected_paths(workspace: Path) -> None:
    adapter = workspace / "models" / "adapters" / "EXP-0001"
    adapter.mkdir(parents=True)
    assert is_protected_path(adapter, workspace)

    empty_cache = workspace / "training" / "cache" / "EXP-0001" / "checkpoints" / "empty"
...
def test_cleanup_does_not_delete_protected_empty_adapter_dir(workspace: Path) -> None:
    empty_adapter = workspace / "models" / "adapters" / "EXP-0001"
...
def test_cleanup_delete_removes_empty_cache_dirs(workspace: Path) -> None:
    empty = workspace / "training" / "cache" / "EXP-0001" / "checkpoints" / "orphan"
