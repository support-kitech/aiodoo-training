#!/usr/bin/env python3
"""AT-7.8 — prepare deterministic Approval smoke subset (does not modify corpus)."""
...
OUT = WORKSPACE / "aiodoo-training/artifacts/at7_approval/smoke"
...
def decision_kind(rec: dict[str, Any]) -> str:
    return str((rec.get("expected_output") or {}).get("decision_kind") or "")
...
def select(
    ...
    *,
    ...
    """Deterministic quotas by decision_kind, then round-robin domain.
    ...
    by_kind: dict[str, list[tuple[str, dict[str, Any], dict[str, Any]]]] = defaultdict(list)
    for item in pool:
        fam = str((item[1].get("metadata") or {}).get("scenario_family") or "")
        if fam in avoid_families:
            continue
        if str(item[1].get("record_type") or "") != AUTHORIZED_TYPE:
            continue
        kind = decision_kind(item[1])
        if kind not in AUTHORIZED_KINDS:
            continue
        by_kind[kind].append(item)
    for k in by_kind:
        by_kind[k].sort(key=lambda x: sort_key(x[0]))

    kinds = sorted(by_kind.keys())
    if not kinds:
        return []

    if kind_quota is None:
        ...
        while sum(quota.values()) > n:
            for k in sorted(quota, key=lambda x: -quota[x]):
                if quota[k] > 0:
                    quota[k] -= 1
                    break
    else:
        quota = {k: int(kind_quota.get(k, 0)) for k in kinds}
        while sum(quota.values()) < n:
            for k in kinds:
                if sum(quota.values()) >= n:
                    break
    ...
    for kind, q in quota.items():
        if q <= 0 or kind not in by_kind:
            continue
        buckets: dict[str, list[tuple[str, dict[str, Any], dict[str, Any]]]] = defaultdict(
            list
        )
        ...
        if len(picked) < q:
            leftovers = sorted(by_kind[kind], key=lambda x: sort_key(x[0]))
    ...
    if len(selected) < n:
        ...
        for rid, rec, _ex in remaining:
            ...
            fam = str((rec.get("metadata") or {}).get("scenario_family") or "")
            fp = fingerprint(rec)
            kind = decision_kind(rec)
            if fam in avoid_families or fam in used_families or fp in used_fps:
                continue
            if str(rec.get("record_type") or "") != AUTHORIZED_TYPE:
                continue
            if kind not in AUTHORIZED_KINDS:
                continue
            selected.append(rid)
...
def main() -> None:
    ...
    for line in (CORPUS / "approval_native.jsonl").read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        rec = json.loads(line)
        assert rec.get("provider_capability") == "approval"
        assert rec.get("record_type") == AUTHORIZED_TYPE
        assert decision_kind(rec) in AUTHORIZED_KINDS
    ...
    for line in (CORPUS / "pack_reasoning.jsonl").read_text(encoding="utf-8").splitlines():
        ...
        assert ex.get("dataset_type") == "approval"
        assert meta.get("provider_capability") == "approval"
        rid = str(meta.get("record_id") or "")
        assert rid in natives
        pack_by_id[rid] = ex

    assert len(pack_by_id) == 162 == len(natives)
    ...
    assert len(train_pool) == 134
    assert len(val_pool) == 14
    assert len(test_pool) == 14

    test_families = {
        str((rec.get("metadata") or {}).get("scenario_family") or "")
        for _, rec, _ in test_pool
    }
    # Preferred ~6/5/5 covering all three Approval behaviors
    train_ids = select(
        train_pool,
        16,
        kind_quota={"approve": 6, "reject": 5, "modify": 5},
    ...
    val_ids = select(
        val_pool,
        4,
        kind_quota={"approve": 2, "reject": 1, "modify": 1},
    ...
    assert len(train_ids) == 16 and len(val_ids) == 4, (len(train_ids), len(val_ids))
    ...
    for rid in train_ids + val_ids:
        ...
        assert rec["record_type"] == AUTHORIZED_TYPE
        kind = decision_kind(rec)
        assert kind in AUTHORIZED_KINDS
        assert kind != "clarify"  # Conversation contamination
    ...
    write_pack(OUT / "data" / "approval_train_smoke.jsonl", train_ids)
    write_pack(OUT / "data" / "approval_validation_smoke.jsonl", val_ids)
    ...
    def dist(ids: list[str]) -> dict[str, Any]:
        ...
        return {
            ...
            "decision_kinds": dict(Counter(e["decision_kind"] for e in entries)),
            "domains": dict(
    ...
    # All three Approval behaviors must reach the path
    assert set(train_dist["decision_kinds"]) == AUTHORIZED_KINDS
    assert train_dist["record_types"] == {AUTHORIZED_TYPE: 16}
    assert sum(train_dist["decision_kinds"].values()) == 16
    ...
    manifest = {
        ...
        "authorized_record_types": [AUTHORIZED_TYPE],
        "authorized_decision_kinds": sorted(AUTHORIZED_KINDS),
        "corpus_version": CORPUS_VERSION,
        "corpus_checksum": checksum,
        "source_pack": "pack_reasoning",
        "split_version": "fp2-split-1.0.0",
        "filter_rule": (
            "metadata.provider_capability==approval AND dataset_type==approval"
        ),
        "native_count": 162,
        "authoritative_train_count": 134,
        "authoritative_validation_count": 14,
        "authoritative_test_count": 14,
        "smoke_train": 16,
        "smoke_validation": 4,
        "selection_method": (
            "deterministic quotas by decision_kind (prefer approve 6 / reject 5 / "
            "modify 5 for train; approve 2 / reject 1 / modify 1 for val) then "
            "round-robin domain ordered by sha256(record_id); unique fingerprints; "
            "at most one record per scenario_family; disjoint from corpus test families"
        ),
        ...
        json.dumps(
            {
                ...
                "val_kinds": val_dist["decision_kinds"],
